# 开发流程

<cite>
**本文引用的文件**   
- [project.config.json](file://project.config.json)
- [miniprogram/app.js](file://miniprogram/app.js)
- [miniprogram/app.json](file://miniprogram/app.json)
- [miniprogram/custom-tab-bar/index.js](file://miniprogram/custom-tab-bar/index.js)
- [miniprogram/pages/home/home.js](file://miniprogram/pages/home/home.js)
- [cloudfunctions/login/index.js](file://cloudfunctions/login/index.js)
- [cloudfunctions/achievements/index.js](file://cloudfunctions/achievements/index.js)
- [cloudfunctions/flashcards/index.js](file://cloudfunctions/flashcards/index.js)
- [cloudfunctions/getCourseList/index.js](file://cloudfunctions/getCourseList/index.js)
- [cloudfunctions/getCourseDetail/index.js](file://cloudfunctions/getCourseDetail/index.js)
- [cloudfunctions/knowledgeMap/index.js](file://cloudfunctions/knowledgeMap/index.js)
- [cloudfunctions/mistakes/index.js](file://cloudfunctions/mistakes/index.js)
- [cloudfunctions/pet/index.js](file://cloudfunctions/pet/index.js)
- [cloudfunctions/quiz/index.js](file://cloudfunctions/quiz/index.js)
- [cloudfunctions/report/index.js](file://cloudfunctions/report/index.js)
- [cloudfunctions/settings/index.js](file://cloudfunctions/settings/index.js)
- [cloudfunctions/aiChat/index.js](file://cloudfunctions/aiChat/index.js)
- [cloudfunctions/aiChat/jest.config.js](file://cloudfunctions/aiChat/jest.config.js)
- [cloudfunctions/aiChat/test/aiChat.test.js](file://cloudfunctions/aiChat/test/aiChat.test.js)
- [cloudfunctions/aiChat/__mocks__/wx-server-sdk.js](file://cloudfunctions/aiChat/__mocks__/wx-server-sdk.js)
- [cloudfunctions/login/jest.config.js](file://cloudfunctions/login/jest.config.js)
- [cloudfunctions/login/test/login.test.js](file://cloudfunctions/login/test/login.test.js)
- [cloudfunctions/login/__mocks__/wx-server-sdk.js](file://cloudfunctions/login/__mocks__/wx-server-sdk.js)
- [cloudfunctions/quiz/jest.config.js](file://cloudfunctions/quiz/jest.config.js)
- [cloudfunctions/quiz/test/quiz.test.js](file://cloudfunctions/quiz/test/quiz.test.js)
- [cloudfunctions/quiz/__mocks__/wx-server-sdk.js](file://cloudfunctions/quiz/__mocks__/wx-server-sdk.js)
- [.github/workflows/ci.yml](file://.github/workflows/ci.yml)
- [docs/superpowers/plans/2026-07-17-course-video-knowledge.md](file://docs/superpowers/plans/2026-07-17-course-video-knowledge.md)
- [docs/superpowers/specs/2026-07-17-course-video-knowledge-design.md](file://docs/superpowers/specs/2026-07-17-course-video-knowledge-design.md)
</cite>

## 更新摘要
**所做更改**   
- 新增Jest测试框架集成与配置指南
- 添加云函数单元测试编写规范与Mock实现说明
- 新增GitHub Actions CI/CD流水线配置与自动化测试流程
- 更新代码提交流程以包含测试要求
- 增强依赖管理章节以涵盖测试依赖

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖分析](#依赖分析)
7. [性能考虑](#性能考虑)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本文件面向团队制定统一的开发工作流程与规范，覆盖Git分支管理、版本控制、代码提交流程、新功能开发步骤、代码审查与合并策略；同时给出项目初始化配置、依赖管理与环境搭建指南；并说明如何添加新页面、新组件与新云函数。文档还包含功能规划文档编写规范与技术设计文档模板，旨在提升团队协作效率与代码质量。**现已集成Jest测试框架和GitHub Actions CI/CD流水线，确保代码质量和自动化部署。**

## 项目结构
本项目为微信小程序 + 云开发（CloudBase）的混合工程：
- miniprogram：小程序前端，包含应用入口、全局配置、自定义TabBar与各业务页面。
- cloudfunctions：云函数集合，按业务域划分目录，每个云函数独立package.json与config.json，部分模块包含Jest测试套件。
- docs：产品与设计文档，包含计划与规格说明。
- prototype：原型文件。
- .github/workflows：GitHub Actions CI/CD配置文件。
- project.config.json：微信开发者工具工程配置。

```mermaid
graph TB
subgraph "小程序前端"
APP["app.js"]
APPJSON["app.json"]
TABBAR["custom-tab-bar/index.js"]
HOME["pages/home/home.js"]
end
subgraph "云函数"
CF_LOGIN["cloudfunctions/login/index.js"]
CF_ACH["cloudfunctions/achievements/index.js"]
CF_FLASH["cloudfunctions/flashcards/index.js"]
CF_COURSE_LIST["cloudfunctions/getCourseList/index.js"]
CF_COURSE_DETAIL["cloudfunctions/getCourseDetail/index.js"]
CF_KMAP["cloudfunctions/knowledgeMap/index.js"]
CF_MISTAKES["cloudfunctions/mistakes/index.js"]
CF_PET["cloudfunctions/pet/index.js"]
CF_QUIZ["cloudfunctions/quiz/index.js"]
CF_REPORT["cloudfunctions/report/index.js"]
CF_SETTINGS["cloudfunctions/settings/index.js"]
CF_AI["cloudfunctions/aiChat/index.js"]
end
subgraph "测试框架"
TEST_LOGIN["login/test/login.test.js"]
TEST_AI["aiChat/test/aiChat.test.js"]
TEST_QUIZ["quiz/test/quiz.test.js"]
MOCK_WX["__mocks__/wx-server-sdk.js"]
JEST_CONFIG["jest.config.js"]
end
subgraph "CI/CD"
GITHUB_ACTIONS[".github/workflows/ci.yml"]
end
APP --> TABBAR
APP --> HOME
HOME --> CF_LOGIN
HOME --> CF_COURSE_LIST
HOME --> CF_COURSE_DETAIL
HOME --> CF_KMAP
HOME --> CF_ACH
HOME --> CF_FLASH
HOME --> CF_MISTAKES
HOME --> CF_PET
HOME --> CF_QUIZ
HOME --> CF_REPORT
HOME --> CF_SETTINGS
HOME --> CF_AI
TEST_LOGIN --> CF_LOGIN
TEST_AI --> CF_AI
TEST_QUIZ --> CF_QUIZ
TEST_LOGIN --> MOCK_WX
TEST_AI --> MOCK_WX
TEST_QUIZ --> MOCK_WX
TEST_LOGIN --> JEST_CONFIG
TEST_AI --> JEST_CONFIG
TEST_QUIZ --> JEST_CONFIG
GITHUB_ACTIONS --> TEST_LOGIN
GITHUB_ACTIONS --> TEST_AI
GITHUB_ACTIONS --> TEST_QUIZ
```

**图表来源**
- [miniprogram/app.js](file://miniprogram/app.js)
- [miniprogram/app.json](file://miniprogram/app.json)
- [miniprogram/custom-tab-bar/index.js](file://miniprogram/custom-tab-bar/index.js)
- [miniprogram/pages/home/home.js](file://miniprogram/pages/home/home.js)
- [cloudfunctions/login/index.js](file://cloudfunctions/login/index.js)
- [cloudfunctions/achievements/index.js](file://cloudfunctions/achievements/index.js)
- [cloudfunctions/flashcards/index.js](file://cloudfunctions/flashcards/index.js)
- [cloudfunctions/getCourseList/index.js](file://cloudfunctions/getCourseList/index.js)
- [cloudfunctions/getCourseDetail/index.js](file://cloudfunctions/getCourseDetail/index.js)
- [cloudfunctions/knowledgeMap/index.js](file://cloudfunctions/knowledgeMap/index.js)
- [cloudfunctions/mistakes/index.js](file://cloudfunctions/mistakes/index.js)
- [cloudfunctions/pet/index.js](file://cloudfunctions/pet/index.js)
- [cloudfunctions/quiz/index.js](file://cloudfunctions/quiz/index.js)
- [cloudfunctions/report/index.js](file://cloudfunctions/report/index.js)
- [cloudfunctions/settings/index.js](file://cloudfunctions/settings/index.js)
- [cloudfunctions/aiChat/index.js](file://cloudfunctions/aiChat/index.js)
- [cloudfunctions/login/test/login.test.js](file://cloudfunctions/login/test/login.test.js)
- [cloudfunctions/aiChat/test/aiChat.test.js](file://cloudfunctions/aiChat/test/aiChat.test.js)
- [cloudfunctions/quiz/test/quiz.test.js](file://cloudfunctions/quiz/test/quiz.test.js)
- [cloudfunctions/login/__mocks__/wx-server-sdk.js](file://cloudfunctions/login/__mocks__/wx-server-sdk.js)
- [cloudfunctions/aiChat/__mocks__/wx-server-sdk.js](file://cloudfunctions/aiChat/__mocks__/wx-server-sdk.js)
- [cloudfunctions/quiz/__mocks__/wx-server-sdk.js](file://cloudfunctions/quiz/__mocks__/wx-server-sdk.js)
- [cloudfunctions/login/jest.config.js](file://cloudfunctions/login/jest.config.js)
- [cloudfunctions/aiChat/jest.config.js](file://cloudfunctions/aiChat/jest.config.js)
- [cloudfunctions/quiz/jest.config.js](file://cloudfunctions/quiz/jest.config.js)
- [.github/workflows/ci.yml](file://.github/workflows/ci.yml)

## 核心组件
- 小程序应用入口与全局配置
  - app.js：应用生命周期与全局状态初始化。
  - app.json：路由、窗口样式、TabBar等全局配置。
  - custom-tab-bar：自定义底部导航逻辑。
- 首页与路由
  - pages/home/home.js：首页逻辑，作为主要跳转入口，调用各云函数完成数据交互。
- 云函数（按业务域）
  - login：登录鉴权相关。
  - achievements：成就系统。
  - flashcards：闪卡学习。
  - getCourseList / getCourseDetail：课程列表与详情。
  - knowledgeMap：知识图谱。
  - mistakes：错题集。
  - pet：宠物系统。
  - quiz：测验。
  - report：报告。
  - settings：设置。
  - aiChat：AI对话。
- **测试框架组件**
  - Jest配置：每个支持测试的云函数包含独立的jest.config.js配置文件。
  - Mock实现：为wx-server-sdk提供模拟实现，便于单元测试。
  - 测试套件：针对login、aiChat、quiz等核心模块的完整测试用例。

**章节来源**
- [miniprogram/app.js](file://miniprogram/app.js)
- [miniprogram/app.json](file://miniprogram/app.json)
- [miniprogram/custom-tab-bar/index.js](file://miniprogram/custom-tab-bar/index.js)
- [miniprogram/pages/home/home.js](file://miniprogram/pages/home/home.js)
- [cloudfunctions/login/index.js](file://cloudfunctions/login/index.js)
- [cloudfunctions/achievements/index.js](file://cloudfunctions/achievements/index.js)
- [cloudfunctions/flashcards/index.js](file://cloudfunctions/flashcards/index.js)
- [cloudfunctions/getCourseList/index.js](file://cloudfunctions/getCourseList/index.js)
- [cloudfunctions/getCourseDetail/index.js](file://cloudfunctions/getCourseDetail/index.js)
- [cloudfunctions/knowledgeMap/index.js](file://cloudfunctions/knowledgeMap/index.js)
- [cloudfunctions/mistakes/index.js](file://cloudfunctions/mistakes/index.js)
- [cloudfunctions/pet/index.js](file://cloudfunctions/pet/index.js)
- [cloudfunctions/quiz/index.js](file://cloudfunctions/quiz/index.js)
- [cloudfunctions/report/index.js](file://cloudfunctions/report/index.js)
- [cloudfunctions/settings/index.js](file://cloudfunctions/settings/index.js)
- [cloudfunctions/aiChat/index.js](file://cloudfunctions/aiChat/index.js)
- [cloudfunctions/login/jest.config.js](file://cloudfunctions/login/jest.config.js)
- [cloudfunctions/aiChat/jest.config.js](file://cloudfunctions/aiChat/jest.config.js)
- [cloudfunctions/quiz/jest.config.js](file://cloudfunctions/quiz/jest.config.js)

## 架构总览
整体采用"小程序前端 + 云函数后端"的解耦架构。小程序通过云开发SDK调用对应云函数，云函数负责业务逻辑与数据访问。**新增Jest测试框架和GitHub Actions CI/CD流水线，实现自动化测试和持续集成。**

```mermaid
sequenceDiagram
participant U as "用户"
participant MP as "小程序前端<br/>home.js"
participant CF as "云函数<br/>login/index.js"
participant DB as "云数据库/对象存储"
participant TEST as "Jest测试框架"
participant CI as "GitHub Actions CI/CD"
U->>MP : 打开小程序/进入首页
MP->>CF : 调用登录接口
CF->>DB : 校验用户信息/写入会话
DB-->>CF : 返回结果
CF-->>MP : 返回登录态/用户信息
MP->>MP : 更新本地状态/跳转目标页
TEST->>CF : 执行单元测试
CF->>TEST : 返回测试结果
CI->>TEST : 触发自动化测试
TEST->>CI : 提交测试报告
```

**图表来源**
- [miniprogram/pages/home/home.js](file://miniprogram/pages/home/home.js)
- [cloudfunctions/login/index.js](file://cloudfunctions/login/index.js)
- [cloudfunctions/login/test/login.test.js](file://cloudfunctions/login/test/login.test.js)
- [.github/workflows/ci.yml](file://.github/workflows/ci.yml)

## 详细组件分析

### Git分支管理策略
- 主分支
  - main：稳定发布分支，仅接受来自release或hotfix的合并。
- 发布分支
  - release/vX.Y.Z：用于预发布与测试，冻结非关键变更。
- 修复分支
  - hotfix/*：针对线上紧急问题修复，快速合并至main与release。
- 功能分支
  - feature/*：所有新功能在此分支开发，完成后发起PR/MR。
- 命名规范
  - 分支名使用小写英文与短横线，语义清晰，如 feature/add-flashcard-page。

### 版本控制规范
- 提交信息格式
  - <type>(<scope>): <subject>
  - type：feat, fix, docs, style, refactor, test, chore
  - scope：模块或云函数名，如 home, login, flashcards
  - subject：简洁描述变更目的
- 示例
  - feat(home): 新增课程列表展示
  - fix(login): 修复登录态过期处理
  - chore(flashcards): 升级依赖包
  - **test(login): 添加登录功能单元测试**

### 代码提交流程
- 拉取最新main分支
- 创建feature分支进行开发
- **编写并运行单元测试（如适用）**
- **确保所有测试通过**
- 自测通过后推送并提交PR/MR
- 至少一名Reviewer审核通过
- **CI/CD流水线自动执行测试**
- 合并到main或release（视发布节奏）
- 打Tag并发布

### 新功能开发步骤
- 需求确认与任务拆分
- 在feature分支实现前后端（小程序页面/组件 + 云函数）
- **为新功能编写Jest单元测试**
- **创建必要的Mock实现**
- 本地联调与单元测试
- **运行完整的测试套件**
- 提交PR/MR并等待评审
- **CI/CD流水线验证**
- 合并后验证回归用例

### 代码审查流程
- 必须包含变更影响范围说明
- **检查测试覆盖率是否满足要求**
- 检查点
  - 正确性：逻辑与边界条件
  - 可读性：命名、注释、结构
  - 可维护性：模块化与复用
  - 安全性：鉴权、输入校验、敏感信息
  - 性能：避免重复请求、合理缓存
  - **测试完整性：单元测试覆盖核心逻辑**
- 评审通过后合并

### 合并策略
- main：保护分支，禁止直接推送
- release：阶段性集成，用于预发布
- hotfix：紧急修复，合并后同步至main与release
- **合并前需通过CI检查（包括自动化测试）**

### 项目初始化配置
- 使用微信开发者工具导入工程
- 在project.config.json中配置AppID、云开发环境ID、基础库版本等
- 首次运行确保已开通云开发并绑定当前环境
- **安装Node.js和npm以支持Jest测试框架**

**章节来源**
- [project.config.json](file://project.config.json)

### 依赖管理
- 云函数依赖
  - 每个云函数目录含package.json与package-lock.json，建议锁定版本
  - 新增依赖需在对应云函数目录执行安装命令并更新锁文件
  - **测试依赖：Jest及相关测试工具应添加到devDependencies中**
- 小程序依赖
  - 第三方库优先使用npm构建，保持最小化引入
  - 避免在多个页面重复引入相同逻辑，抽取公共utils
- **测试依赖管理**
  - Jest测试框架：在每个需要测试的云函数目录中单独配置
  - Mock实现：为外部依赖（如wx-server-sdk）提供模拟实现
  - 测试配置文件：jest.config.js统一管理测试行为

**章节来源**
- [cloudfunctions/login/package.json](file://cloudfunctions/login/package.json)
- [cloudfunctions/achievements/package.json](file://cloudfunctions/achievements/package.json)
- [cloudfunctions/flashcards/package.json](file://cloudfunctions/flashcards/package.json)
- [cloudfunctions/getCourseList/package.json](file://cloudfunctions/getCourseList/package.json)
- [cloudfunctions/getCourseDetail/package.json](file://cloudfunctions/getCourseDetail/package.json)
- [cloudfunctions/knowledgeMap/package.json](file://cloudfunctions/knowledgeMap/package.json)
- [cloudfunctions/mistakes/package.json](file://cloudfunctions/mistakes/package.json)
- [cloudfunctions/pet/package.json](file://cloudfunctions/pet/package.json)
- [cloudfunctions/quiz/package.json](file://cloudfunctions/quiz/package.json)
- [cloudfunctions/report/package.json](file://cloudfunctions/report/package.json)
- [cloudfunctions/settings/package.json](file://cloudfunctions/settings/package.json)
- [cloudfunctions/aiChat/package.json](file://cloudfunctions/aiChat/package.json)
- [cloudfunctions/login/jest.config.js](file://cloudfunctions/login/jest.config.js)
- [cloudfunctions/aiChat/jest.config.js](file://cloudfunctions/aiChat/jest.config.js)
- [cloudfunctions/quiz/jest.config.js](file://cloudfunctions/quiz/jest.config.js)

### 环境搭建指南
- 安装微信开发者工具并登录
- 在project.config.json中配置云开发环境ID
- 在开发者工具中开启云开发并选择对应环境
- 首次部署云函数：右键云函数目录 -> 上传并部署：云端安装依赖
- **安装Node.js（推荐LTS版本）以支持Jest测试框架**
- **安装npm依赖：cd cloudfunctions/[function-name] && npm install**
- **运行测试：npm test**

**章节来源**
- [project.config.json](file://project.config.json)

### 如何添加新页面
- 在miniprogram/pages下新建页面目录，包含js/json/wxml/wxss
- 在app.json的pages数组中添加路由
- 如需加入TabBar，在app.json的tabBar.list中注册
- 在custom-tab-bar中维护选中态与跳转逻辑

**章节来源**
- [miniprogram/app.json](file://miniprogram/app.json)
- [miniprogram/custom-tab-bar/index.js](file://miniprogram/custom-tab-bar/index.js)

### 如何添加新组件
- 在miniprogram/components下新建组件目录，包含同名js/json/wxml/wxss
- 在页面json中引用组件
- 定义props、methods与事件，保证高内聚低耦合

### 如何添加新云函数
- 在cloudfunctions下新建云函数目录，包含index.js、config.json、package.json
- 在index.js中实现API逻辑，遵循统一错误码与返回结构
- 在小程序侧通过云开发SDK调用该云函数
- 部署云函数并在开发者工具中测试
- **可选：为云函数添加Jest测试套件**
  - 创建test目录和jest.config.js配置文件
  - 编写单元测试用例
  - 为外部依赖创建Mock实现

**章节来源**
- [cloudfunctions/login/index.js](file://cloudfunctions/login/index.js)
- [cloudfunctions/achievements/index.js](file://cloudfunctions/achievements/index.js)
- [cloudfunctions/flashcards/index.js](file://cloudfunctions/flashcards/index.js)
- [cloudfunctions/getCourseList/index.js](file://cloudfunctions/getCourseList/index.js)
- [cloudfunctions/getCourseDetail/index.js](file://cloudfunctions/getCourseDetail/index.js)
- [cloudfunctions/knowledgeMap/index.js](file://cloudfunctions/knowledgeMap/index.js)
- [cloudfunctions/mistakes/index.js](file://cloudfunctions/mistakes/index.js)
- [cloudfunctions/pet/index.js](file://cloudfunctions/pet/index.js)
- [cloudfunctions/quiz/index.js](file://cloudfunctions/quiz/index.js)
- [cloudfunctions/report/index.js](file://cloudfunctions/report/index.js)
- [cloudfunctions/settings/index.js](file://cloudfunctions/settings/index.js)
- [cloudfunctions/aiChat/index.js](file://cloudfunctions/aiChat/index.js)

### Jest测试框架集成指南
**新增** 项目已集成Jest测试框架，支持云函数的单元测试。

#### 测试框架配置
- 每个需要测试的云函数目录包含独立的jest.config.js配置文件
- 配置文件定义了测试文件匹配模式、测试环境和Mock配置
- 支持异步测试和Promise处理

#### Mock实现
- 为wx-server-sdk提供统一的Mock实现
- Mock文件位于__mocks__目录下
- 模拟云开发SDK的核心功能，便于单元测试

#### 测试套件结构
- 测试文件位于test目录下，命名为[module].test.js
- 使用describe和it组织测试用例
- 支持异步测试和错误场景测试

#### 运行测试
- 在云函数目录中执行：npm test
- 支持测试覆盖率统计
- 可在CI/CD流水线中自动执行

**章节来源**
- [cloudfunctions/login/jest.config.js](file://cloudfunctions/login/jest.config.js)
- [cloudfunctions/aiChat/jest.config.js](file://cloudfunctions/aiChat/jest.config.js)
- [cloudfunctions/quiz/jest.config.js](file://cloudfunctions/quiz/jest.config.js)
- [cloudfunctions/login/__mocks__/wx-server-sdk.js](file://cloudfunctions/login/__mocks__/wx-server-sdk.js)
- [cloudfunctions/aiChat/__mocks__/wx-server-sdk.js](file://cloudfunctions/aiChat/__mocks__/wx-server-sdk.js)
- [cloudfunctions/quiz/__mocks__/wx-server-sdk.js](file://cloudfunctions/quiz/__mocks__/wx-server-sdk.js)
- [cloudfunctions/login/test/login.test.js](file://cloudfunctions/login/test/login.test.js)
- [cloudfunctions/aiChat/test/aiChat.test.js](file://cloudfunctions/aiChat/test/aiChat.test.js)
- [cloudfunctions/quiz/test/quiz.test.js](file://cloudfunctions/quiz/test/quiz.test.js)

### GitHub Actions CI/CD流水线
**新增** 项目已建立GitHub Actions CI/CD流水线，实现自动化测试和部署。

#### 流水线配置
- 配置文件位于.github/workflows/ci.yml
- 支持多节点并行测试
- 自动检测代码变更并触发测试

#### 自动化测试
- 在所有支持的云函数上运行Jest测试
- 收集测试报告和覆盖率
- 失败时阻止合并请求

#### 持续集成
- 每次推送代码自动触发测试
- 提供详细的测试日志和失败信息
- 支持测试结果的可视化展示

**章节来源**
- [.github/workflows/ci.yml](file://.github/workflows/ci.yml)

### 功能规划文档编写规范
- 文档存放位置：docs/superpowers/plans
- 命名规则：YYYY-MM-DD-主题.md
- 内容要点
  - 背景与目标
  - 用户故事与验收标准
  - 范围与约束
  - 里程碑与排期
  - 风险与应对
  - 指标与度量

**章节来源**
- [docs/superpowers/plans/2026-07-17-course-video-knowledge.md](file://docs/superpowers/plans/2026-07-17-course-video-knowledge.md)

### 技术设计文档模板
- 文档存放位置：docs/superpowers/specs
- 命名规则：YYYY-MM-DD-主题-design.md
- 内容要点
  - 概述与范围
  - 架构与模块划分
  - 接口定义（请求/响应）
  - 数据模型与关系
  - 安全与权限
  - 性能与容量评估
  - **测试策略与测试用例设计**
  - 上线与回滚方案

**章节来源**
- [docs/superpowers/specs/2026-07-17-course-video-knowledge-design.md](file://docs/superpowers/specs/2026-07-17-course-video-knowledge-design.md)

## 依赖分析
- 前端依赖
  - 小程序入口与路由由app.js与app.json管理
  - 自定义TabBar由custom-tab-bar/index.js维护
  - 页面间跳转与数据获取集中在home.js作为示例入口
- 后端依赖
  - 各云函数独立部署，彼此无直接依赖，通过小程序统一调用
  - 云函数内部依赖通过各自package.json声明
- **测试依赖**
  - Jest测试框架：为云函数提供单元测试能力
  - Mock实现：模拟外部依赖以便隔离测试
  - CI/CD工具：GitHub Actions实现自动化测试和部署

```mermaid
graph LR
APPJS["app.js"] --> APPJSON["app.json"]
APPJSON --> TABBAR["custom-tab-bar/index.js"]
HOME["pages/home/home.js"] --> CFLOGIN["cloudfunctions/login/index.js"]
HOME --> CFCOURSELIST["cloudfunctions/getCourseList/index.js"]
HOME --> CFCOURSEDETAIL["cloudfunctions/getCourseDetail/index.js"]
HOME --> CFKMAP["cloudfunctions/knowledgeMap/index.js"]
HOME --> CFACH["cloudfunctions/achievements/index.js"]
HOME --> CFFLASH["cloudfunctions/flashcards/index.js"]
HOME --> CFMISTAKES["cloudfunctions/mistakes/index.js"]
HOME --> CFPET["cloudfunctions/pet/index.js"]
HOME --> CFQUIZ["cloudfunctions/quiz/index.js"]
HOME --> CFREPORT["cloudfunctions/report/index.js"]
HOME --> CFSETTINGS["cloudfunctions/settings/index.js"]
HOME --> CFAI["cloudfunctions/aiChat/index.js"]
TEST_LOGIN["login/test/login.test.js"] --> CFLOGIN
TEST_AI["aiChat/test/aiChat.test.js"] --> CFAI
TEST_QUIZ["quiz/test/quiz.test.js"] --> CFQUIZ
TEST_LOGIN --> MOCK_WX["__mocks__/wx-server-sdk.js"]
TEST_AI --> MOCK_WX
TEST_QUIZ --> MOCK_WX
CI["GitHub Actions CI/CD"] --> TEST_LOGIN
CI --> TEST_AI
CI --> TEST_QUIZ
```

**图表来源**
- [miniprogram/app.js](file://miniprogram/app.js)
- [miniprogram/app.json](file://miniprogram/app.json)
- [miniprogram/custom-tab-bar/index.js](file://miniprogram/custom-tab-bar/index.js)
- [miniprogram/pages/home/home.js](file://miniprogram/pages/home/home.js)
- [cloudfunctions/login/index.js](file://cloudfunctions/login/index.js)
- [cloudfunctions/getCourseList/index.js](file://cloudfunctions/getCourseList/index.js)
- [cloudfunctions/getCourseDetail/index.js](file://cloudfunctions/getCourseDetail/index.js)
- [cloudfunctions/knowledgeMap/index.js](file://cloudfunctions/knowledgeMap/index.js)
- [cloudfunctions/achievements/index.js](file://cloudfunctions/achievements/index.js)
- [cloudfunctions/flashcards/index.js](file://cloudfunctions/flashcards/index.js)
- [cloudfunctions/mistakes/index.js](file://cloudfunctions/mistakes/index.js)
- [cloudfunctions/pet/index.js](file://cloudfunctions/pet/index.js)
- [cloudfunctions/quiz/index.js](file://cloudfunctions/quiz/index.js)
- [cloudfunctions/report/index.js](file://cloudfunctions/report/index.js)
- [cloudfunctions/settings/index.js](file://cloudfunctions/settings/index.js)
- [cloudfunctions/aiChat/index.js](file://cloudfunctions/aiChat/index.js)
- [cloudfunctions/login/test/login.test.js](file://cloudfunctions/login/test/login.test.js)
- [cloudfunctions/aiChat/test/aiChat.test.js](file://cloudfunctions/aiChat/test/aiChat.test.js)
- [cloudfunctions/quiz/test/quiz.test.js](file://cloudfunctions/quiz/test/quiz.test.js)
- [cloudfunctions/login/__mocks__/wx-server-sdk.js](file://cloudfunctions/login/__mocks__/wx-server-sdk.js)
- [cloudfunctions/aiChat/__mocks__/wx-server-sdk.js](file://cloudfunctions/aiChat/__mocks__/wx-server-sdk.js)
- [cloudfunctions/quiz/__mocks__/wx-server-sdk.js](file://cloudfunctions/quiz/__mocks__/wx-server-sdk.js)
- [.github/workflows/ci.yml](file://.github/workflows/ci.yml)

## 性能考虑
- 减少不必要的网络请求，合并接口或引入缓存
- 云函数幂等设计与重试策略
- 图片与静态资源压缩与CDN加速
- 小程序分包加载与按需引入
- 避免长事务与阻塞操作，异步化处理耗时任务
- **测试性能优化：合理使用Mock减少外部依赖调用**
- **CI/CD性能：并行执行测试任务提高构建速度**

## 故障排查指南
- 常见问题定位
  - 云函数未部署或权限不足：检查云开发环境与函数部署状态
  - 小程序调用失败：核对接口路径、参数与返回结构
  - 路由异常：检查app.json的pages与tabBar配置
  - **测试失败：检查Jest配置和Mock实现是否正确**
  - **CI/CD失败：查看GitHub Actions日志和测试报告**
- 日志与调试
  - 使用开发者工具控制台查看网络请求与错误堆栈
  - 在云函数中输出必要日志以便追踪
  - **使用Jest调试模式：node --inspect-brk node_modules/.bin/jest**
- 回滚与应急
  - 保留最近可用版本标签，必要时回滚至上一稳定版本
  - **利用CI/CD流水线快速重新部署已知稳定的版本**

## 结论
通过统一的分支策略、提交规范、审查流程与文档模板，结合清晰的架构与依赖关系，可有效提升团队协作效率与代码质量。**现已集成Jest测试框架和GitHub Actions CI/CD流水线，实现了自动化测试和持续集成，进一步保障了代码质量和交付效率。**建议在后续迭代中持续完善自动化检查与发布流水线，扩大测试覆盖率，降低人为失误与回归风险。

## 附录
- 术语表
  - PR/MR：Pull Request / Merge Request
  - CI/CD：持续集成/持续交付
  - 云函数：Serverless函数，承载后端逻辑
  - **Jest：JavaScript测试框架，用于单元测试**
  - **Mock：模拟对象，用于隔离测试环境**
  - **CI/CD流水线：自动化测试和部署流程**
- 参考链接
  - 微信开发者工具文档
  - 云开发官方文档
  - **Jest官方文档**
  - **GitHub Actions官方文档**