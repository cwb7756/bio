...
## 更新摘要
**变更内容**   
- 新增了知识图谱（knowledgeMap）和笔记本（notebook）云函数实现，扩展了学习生态系统的核心能力
...
## 引言
本文件面向基于微信云开发的微服务化云函数架构，系统化阐述职责划分、模块化设计、生命周期与事件驱动、异步处理模式、云函数间通信与数据共享策略、状态管理、部署配置与环境变量管理、版本控制以及最佳实践与性能优化建议。文档以仓库中的云函数目录为事实依据，结合小程序端调用路径进行端到端说明，帮助开发者快速理解并落地高质量云函数体系。本次更新重点介绍了新增的知识图谱和笔记本功能、增强的细粒度数据库权限控制、改进的错误处理机制、优化的日志记录和完善的包管理配置。

## 项目结构
本项目采用"按能力域拆分"的云函数组织方式，每个云函数对应一个独立的能力边界（如登录、课程、AI 对话、错题、报告、知识图谱、笔记本等），并通过小程序页面发起调用。根级配置文件用于声明云环境、函数入口与基础运行参数。
...
- **新增功能**：knowledgeMap 提供知识图谱构建和管理能力，notebook 提供个人学习笔记管理功能。
...
**更新** 所有核心云函数现在都具备统一的错误处理模式和详细的日志记录，提升了系统的可观测性和稳定性。新增的知识图谱和笔记本功能进一步完善了学习生态系统。
...
- **新功能性能考虑**：aiCourseware、feedback、knowledgeMap 和 notebook 云函数在设计时充分考虑了性能因素，采用异步处理和缓存策略确保高并发下的稳定性。
